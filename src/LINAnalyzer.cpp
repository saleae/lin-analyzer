...
void LINAnalyzer::WorkerThread()
{
	...
	for( ; ; )
	{
		...
		{
			byteFrame.mData1 = ByteFrame( byteFrame.mStartingSampleInclusive, byteFrame.mEndingSampleInclusive, byteFramingError, is_data_really_break );
		}
		...
		switch( mFrameState )
		{
			...
			case LINAnalyzerResults::headerPID:				// expecting PID.
				{
					...
					{
						mChecksum.add( byteFrame.mData1 ); //We only add the PID byte to the checksum IF we're using version 2 AND we're not using one of the classic identifers.
					}
				}
				break;
			// LIN Response
			...
			case LINAnalyzerResults::responseData:			// expecting response data.
				if ( nDataBytes >= 8 || mChecksum.result() == byteFrame.mData1 )
				{
					...
					if( nDataBytes >= 8 )
						ready_to_save = true; //we should not terminate a packet unless we're sure that the packet is over, and we're not sure yet, except when there are 8 data bytes.
					else
...
U8 LINAnalyzer::GetBreakField( S64& startingSample, S64& endingSample, bool& framingError )
{
	...
	U32 min_break_field_low_bits = 13; //as per the spec of LIN. at least 13 bits at master speed, but receiver only needs it to be 11 or more.
...
U8 LINAnalyzer::ByteFrame( S64& startingSample, S64& endingSample, bool& framingError, bool& is_break_field )
{
	...
	else
	{
		...
		bool all_13_clear = !(mSerial->WouldAdvancingCauseTransition( SamplesPerBit() * 3 )); //checks the remaining 3 bits to make sure they are high.
		
		mSerial->AdvanceToNextEdge(); //we're high again, and at the end of the break frame!
		//verify that we've found a stop bit.
		bool high_bit_present = !mSerial->WouldAdvancingCauseTransition( SamplesPerBit() * 0.5 ); //if there are no transitions, that means it's high in the center of the bit field, and we're good!
		...
		{
			/*mResults->AddMarker( mSerial->GetSampleNumber(), mSerial->GetBitState() == BIT_HIGH ? AnalyzerResults::One : AnalyzerResults::Zero, mSettings->mInputChannel );
			for( U32 i = 0; i < 4; ++i )
			{
				Advance( 1 );
				mResults->AddMarker( mSerial->GetSampleNumber(), mSerial->GetBitState() == BIT_HIGH ? AnalyzerResults::One : AnalyzerResults::Zero, mSettings->mInputChannel );
